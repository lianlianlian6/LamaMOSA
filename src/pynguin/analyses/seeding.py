...
class InitialPopulationProvider:
        ...
        try:
            if len(result) > 0:
                ...
                with result[0].open(mode="r", encoding="utf-8") as module_file:
                    return ast.parse(module_file.read())          ###返回待测单元的解析树
            else:
    ...
    def collect_testcases(self, module_path: AnyStr | os.PathLike[AnyStr]) -> None:
        ...
        if tree is None:
            ...
            return
        transformer = AstToTestCaseTransformer(       ####将ast转换为测试用例
            self._test_cluster,
...
def create_assign_stmt(
    ...
    if isinstance(value, ast.Constant):      ####根据assign.value的具体值，进行分类讨论
...
def create_stmt_from_call(
    ...
    """
    try:
        call.func.attr  # type: ignore[attr-defined]  # noqa: B018  ###方法调用有attr属性，没有则是普通函数调用
    except AttributeError:
    ...
    for obj in objs_under_test:
        if isinstance(obj, GenericConstructor):
            ...
            try:
                ...
                return None
        # elif isinstance(obj, GenericFunction) and call_name == obj.function_name:  ##即只有obj.method_name==obj.function_name才返回obj，这不对
        elif isinstance(obj, GenericFunction):
            return obj
    return None    #####从给定的 AST 调用节点中找到相应的可调用对象（构造函数、方法或函数）
